...
log_lik_data |> 
  ...
  labs(title = "Log-Likelihood for Bernoulli Data",
       ...
       x = "Parameter (p)", y = "Log-Likelihood ℓ(p)") +
  annotate("text", x = mle_p + 0.05, y = x * log(mle_p) + (n - x) * log(1 - mle_p) - 2,
           label = str_glue("Maximum at\np̂ = {mle_p}"), color = "red", size = 5)
```
...
bind_rows(
  ...
  labs(title = "Log-Likelihood Curvature: More Data = More Information",
       subtitle = "Both have same MLE (p̂ = 0.76), but n = 100 is more curved",
...
## Motivation: Constructing Estimators
...
1.  **Method of Moments** (this lesson)
...
2.  **Fisher Information:** $I(\theta) = E[-\ell''(\theta)]$
...
3.  **Cramér-Rao Lower Bound:**
    ...
    -   Efficient estimators achieve the bound

4.  **Method of Moments:** Equate population and sample moments
...
-   Exercise 7.51: Geometric distribution - Fisher information
-   Exercise 7.52: Poisson - efficiency
-   Exercise 7.54: Exponential - CRLB and efficiency
-   Exercise 7.55: Normal - efficient estimator
-   Exercise 7.25-27: Method of moments applications
:::
...
-   Finding the MLE: maximize $L(\theta)$ or $\ell(\theta)$
-   Properties: invariance, consistency, asymptotic normality
-   MLEs are asymptotically efficient!
-   Comparison of MLE vs MME

## References

::: nonincremental
-   Devore, Berk, and Carlton. *Modern Mathematical Statistics with Applications* (Springer). Chapters 7.1, 7.2, 7.4
