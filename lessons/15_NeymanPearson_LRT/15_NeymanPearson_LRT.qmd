...
tibble(y = 0:30) |>
  ...
  mutate(sample_size = fct_rev(as.factor(sample_size))) |>
  ggplot(aes(x = y, y = prob, fill = dist)) +
  geom_col(position = "identity", alpha = 0.5, width = 0.8) +
  geom_vline(data = tibble(sample_size = as.factor(c("n = 5", "n = 10")), 
                            cutoff = c(10, 16)),
             aes(xintercept = cutoff - 0.5), linetype = "dashed", 
             color = "red", linewidth = 1) +
  facet_wrap(~sample_size, scales = "free") +
  scale_fill_manual(values = c("steelblue", "coral", "pink")) +
