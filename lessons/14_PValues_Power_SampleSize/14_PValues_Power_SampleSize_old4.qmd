...
#| fig-width: 12
set.seed(551)
# Simulate 50 P-values: 40 from null (Uniform), 10 from alternative (concentrated near 0)
m <- 50
p_null <- runif(40)
p_alt <- rbeta(10, 1, 20)  # Concentrated near 0
...
bh_data <- tibble(
  ...
  reject = pvalue <= bh_line[max(which(pvalues_sorted <= bh_threshold))]
)

# Find the BH cutoff
k_star <- max(which(pvalues_sorted <= bh_threshold))

ggplot(bh_data, aes(x = rank, y = pvalue)) +
  geom_point(aes(color = reject), size = 3) +
  geom_line(aes(y = bh_line), color = "steelblue", linewidth = 1, linetype = "dashed") +
  geom_vline(xintercept = k_star, color = "red", linetype = "dotted", linewidth = 1) +
  scale_color_manual(values = c("gray50", "firebrick"), 
                     labels = c("Not rejected", "Rejected")) +
  annotate("text", x = k_star + 2, y = 0.5, hjust = 0, color = "red", size = 5,
           label = paste0("k* = ", k_star, " rejections")) +
  annotate("text", x = 40, y = 0.07, color = "steelblue", size = 5,
           label = "BH threshold line: (k/m) * q") +
  labs(x = "Rank (ordered P-values)",
       y = "P-value",
       title = "Benjamini-Hochberg Procedure (FDR = 0.10)",
       subtitle = "Reject all hypotheses below the largest P-value that falls under the threshold line",
       color = "")
```
