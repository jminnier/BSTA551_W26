...
**Fisher's exact test** is a statistical test used to determine whether there is a significant association between two categorical variables in a $2 \times 2$ contingency table.
...
**The question:** Is the proportion with Outcome+ different between Group A and Group B?
...
| **Sample size** | Valid for any $n$ | Requires "large" $n$ (expected counts $\geq 5$) |
| **Computation** | Enumerates all tables | Simple formula |
| **Sparse tables** | Handles well | May be unreliable |
| **One-sided tests** | Natural | Awkward |
...
To understand Fisher's exact test deeply, we need to address a fundamental question:

. . .

**How do we compute a P-value when the null hypothesis doesn't fully specify the distribution?**

. . .

Testing $H_0$: "Group and outcome are independent" does **not** tell us:

- What proportion of subjects are in Group A
- What proportion have the outcome

These **nuisance parameters** affect the distribution of any test statistic, yet we don't know their values!
...
adverse_table <- matrix(c(2, 8, 18, 12), nrow = 2, byrow = FALSE,
...
```{r}
#| echo: true
# Verify: Fisher's P-value via permutation simulation (one-sided version)
set.seed(551)
n_perm <- 10000

# Reconstruct individual-level data matching adverse_table:
# Treatment: 2 adverse, 18 none; Placebo: 8 adverse, 12 none
treatment <- c(rep(1, 20), rep(0, 20))  # 20 treatment, 20 placebo
adverse <- c(rep(1, 2), rep(0, 18),     # Treatment: 2 adverse, 18 not
             rep(1, 8), rep(0, 12))      # Placebo: 8 adverse, 12 not

# Observed statistic: treatment patients with adverse events
obs_stat <- sum(treatment * adverse)  # = 2
cat("Observed statistic (treatment + adverse):", obs_stat, "\n")

# Permutation: shuffle adverse labels, count treatment patients with adverse
perm_stats <- replicate(n_perm, {
  sum(treatment * sample(adverse))
})

# One-sided P-value: P(X <= 2) - treatment has fewer adverse events
cat("Simulated P-value (one-sided):", mean(perm_stats <= obs_stat), "\n")
cat("Fisher's exact P-value (one-sided):", 
    round(fisher.test(adverse_table, alternative = "less")$p.value, 4), "\n")
```
...
```{r}
#| echo: true
# Create the contingency table from raw data
patients <- tibble(
  patient_id = 1:45,
  variant = c(rep("Present", 18), rep("Absent", 27)),
  response = c(
    # Variant present: 12 responders, 6 non-responders
    rep("Responder", 12), rep("Non-responder", 6),
    # Variant absent: 8 responders, 19 non-responders
    rep("Responder", 8), rep("Non-responder", 19)
  )
)

# Cross-tabulation
response_table <- table(patients$variant, patients$response)
response_table
```
...
```{r}
#| echo: true
# If we have a directional hypothesis (variant improves response)
fisher_greater <- fisher.test(response_table, alternative = "greater")
fisher_less <- fisher.test(response_table, alternative = "less")

cat("Two-sided P-value:", round(fisher_result$p.value, 4), "\n")
cat("One-sided (OR > 1):", round(fisher_greater$p.value, 4), "\n")
cat("One-sided (OR < 1):", round(fisher_less$p.value, 4), "\n")
```
...
```{r}
#| echo: true
# Chi-squared test (asymptotic approximation)
...
cat("Fisher's exact test:", round(fisher_result$p.value, 4), "\n")
cat("Chi-squared test:   ", round(chisq_result$p.value, 4), "\n")
...
# Multiple Testing and P-Value Adjustment {background-color="#c0392b"}
...
$$\text{Expected false discoveries} = m \times \alpha = 20{,}000 \times 0.05 = 1{,}000$$
...
$$\text{FWER} = 1 - P(\text{no false rejections}) = 1 - (1 - \alpha)^m$$

. . .

```{r}
#| echo: true
# FWER as a function of m for alpha = 0.05
m_tests <- c(1, 2, 5, 10, 20, 50, 100)
fwer <- 1 - (1 - 0.05)^m_tests
tibble(m = m_tests, FWER = round(fwer, 3)) |> 
  pivot_wider(names_from = m, values_from = FWER) |>
  knitr::kable(col.names = paste0("m = ", m_tests))
```
...
ggplot(fwer_data, aes(x = m, y = FWER)) +
  geom_line(linewidth = 1.5, color = "firebrick") +
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "steelblue", linewidth = 1) +
  geom_hline(yintercept = 0.5, linetype = "dotted", color = "gray50") +
  annotate("text", x = 80, y = 0.08, label = "Nominal alpha = 0.05", 
           color = "steelblue", size = 5) +
  annotate("text", x = 80, y = 0.53, label = "FWER = 50%", color = "gray50", size = 5) +
  annotate("point", x = 14, y = 1 - (1 - 0.05)^14, color = "firebrick", size = 3) +
  annotate("text", x = 18, y = 0.55, label = "At m = 14, FWER > 50%", 
           color = "firebrick", size = 5, hjust = 0) +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  labs(x = "Number of Tests (m)",
       y = "Family-Wise Error Rate",
       title = "FWER Inflation with Multiple Testing",
       subtitle = "Using individual alpha = 0.05 for each test (assuming independence)")
```
...
To control the FWER at level $\alpha$ when conducting $m$ tests, reject each individual hypothesis only if its P-value satisfies:
...
**Theoretical justification (Boole's inequality):**
...
```{r}
#| echo: true
# Raw P-values from 5 tests
biomarkers <- c("CRP", "IL-6", "TNF-alpha", "Cortisol", "Fibrinogen")
raw_pvalues <- c(0.008, 0.023, 0.041, 0.067, 0.12)

# Bonferroni adjustment
m <- length(raw_pvalues)
bonf_adjusted <- pmin(raw_pvalues * m, 1)  # p.adjust does this

# Using R's built-in function
bonf_adjusted_r <- p.adjust(raw_pvalues, method = "bonferroni")

tibble(
  Biomarker = biomarkers,
  `Raw P-value` = raw_pvalues,
  `Bonferroni Adjusted` = round(bonf_adjusted_r, 3),
  `Significant (alpha=0.05)` = ifelse(bonf_adjusted_r <= 0.05, "Yes", "No")
) |> knitr::kable()
```

. . .

Only CRP remains significant after Bonferroni correction!

## Limitations of Bonferroni

::: callout-warning
## Bonferroni is Conservative
The Bonferroni correction controls FWER at level **at most** $\alpha$, but often the actual FWER is much smaller, leading to reduced **power**.
:::
...
3. Reject all hypotheses with $p_{(i)} \leq p_{(k)}$ (i.e., reject $H_{(1)}, \ldots, H_{(k)}$)
:::

. . .

**Adjusted P-values:** The BH-adjusted P-value for the $i$-th ordered hypothesis is:
...
#| fig-width: 12
set.seed(2026)
# Simulate 100 P-values: 70 from null (Uniform), 30 from alternative (concentrated near 0)
m <- 100
n_null <- 70
n_alt <- 30
p_null <- runif(n_null)
p_alt <- rbeta(n_alt, 0.5, 15)  # Strongly concentrated near 0
pvalues <- c(p_null, p_alt)

# Sort and create BH threshold
pvalues_sorted <- sort(pvalues)
k <- 1:m
q <- 0.10
bh_threshold <- (k / m) * q

# Find the BH cutoff: largest k where p_(k) <= (k/m)*q
below_line <- which(pvalues_sorted <= bh_threshold)
k_star <- ifelse(length(below_line) > 0, max(below_line), 0)

bh_data <- tibble(
  rank = k,
  pvalue = pvalues_sorted,
  bh_line = bh_threshold,
  reject = rank <= k_star
)

ggplot(bh_data, aes(x = rank, y = pvalue)) +
  geom_line(aes(y = bh_line), color = "steelblue", linewidth = 1.2, linetype = "dashed") +
  geom_point(aes(color = reject), size = 2.5, alpha = 0.8) +
  geom_segment(x = k_star, xend = k_star, y = 0, yend = pvalues_sorted[k_star],
               color = "red", linetype = "dotted", linewidth = 1) +
  geom_segment(x = 0, xend = k_star, y = pvalues_sorted[k_star], yend = pvalues_sorted[k_star],
               color = "red", linetype = "dotted", linewidth = 1) +
  scale_color_manual(values = c("gray50", "firebrick"), 
                     labels = c("Not rejected", "Rejected")) +
  annotate("text", x = k_star + 3, y = 0.35, hjust = 0, color = "red", size = 5,
           label = paste0("k* = ", k_star, " rejections")) +
  annotate("text", x = 70, y = 0.06, color = "steelblue", size = 5,
           label = "BH threshold: (k/m) × q") +
  annotate("point", x = k_star, y = pvalues_sorted[k_star], 
           color = "red", size = 4, shape = 1, stroke = 2) +
  annotate("text", x = k_star + 3, y = pvalues_sorted[k_star] + 0.03, hjust = 0,
           color = "red", size = 4,
           label = paste0("p(", k_star, ") = ", round(pvalues_sorted[k_star], 3))) +
  coord_cartesian(ylim = c(0, 0.5)) +
  labs(x = "Rank (ordered P-values)",
       y = "P-value",
       title = "Benjamini-Hochberg Procedure (FDR = 0.10)",
       subtitle = "Reject all hypotheses with rank ≤ k*, where k* is the largest k with p(k) ≤ (k/m)q",
       color = "") +
...
adjustments <- tibble(
  ...
  BH = p.adjust(raw_pvalues, method = "BH"),
  BY = p.adjust(raw_pvalues, method = "BY")
)
...
**BH allows more discoveries** than Bonferroni while still controlling false discoveries.
...
```{r}
#| echo: true
# Simulate a genomics-style analysis
set.seed(551)
m <- 1000  # 1000 genes
n_true_effects <- 50  # 50 genes truly differentially expressed

# Generate P-values
pvals <- c(
  rbeta(n_true_effects, 0.5, 10),  # True effects: small P-values
  runif(m - n_true_effects)         # Null: uniform P-values
)
...
# Count discoveries at alpha/q = 0.05
cat("Number of 'significant' results (adjusted P <= 0.05):\n")
cat("  Raw (no adjustment):", sum(pvals <= 0.05), "\n")
cat("  Bonferroni:         ", sum(results$bonferroni <= 0.05), "\n")
cat("  Holm:               ", sum(results$holm <= 0.05), "\n")
cat("  BH (FDR):           ", sum(results$BH <= 0.05), "\n")
cat("\nTrue positives (out of", n_true_effects, "true effects):\n")
cat("  BH discoveries that are true effects:", 
    sum(results$BH[1:n_true_effects] <= 0.05), "\n")
```
...
set.seed(551)
n_sims <- 1000
m <- 100  # tests per simulation
pi0 <- 0.9  # proportion of true nulls

sim_results <- map_dfr(1:n_sims, function(sim) {
  # Generate P-values
  n_null <- round(m * pi0)
  n_alt <- m - n_null
  pvals <- c(runif(n_null), rbeta(n_alt, 0.3, 5))
  true_null <- c(rep(TRUE, n_null), rep(FALSE, n_alt))
  
  # Apply corrections
  bonf_reject <- p.adjust(pvals, "bonferroni") <= 0.05
  bh_reject <- p.adjust(pvals, "BH") <= 0.05
  
  tibble(
    sim = sim,
    # FWER: any false rejection among true nulls?
    bonf_any_fp = any(bonf_reject & true_null),
    bh_any_fp = any(bh_reject & true_null),
    # FDR: proportion of rejections that are false
    bonf_fdr = sum(bonf_reject & true_null) / max(sum(bonf_reject), 1),
    bh_fdr = sum(bh_reject & true_null) / max(sum(bh_reject), 1),
    # Power: proportion of true alternatives detected
    bonf_power = sum(bonf_reject & !true_null) / n_alt,
    bh_power = sum(bh_reject & !true_null) / n_alt
  )
})
...
summary_stats <- sim_results |>
  summarise(
    `Bonferroni FWER` = mean(bonf_any_fp),
    `BH FWER` = mean(bh_any_fp),
    `Bonferroni FDR` = mean(bonf_fdr),
    `BH FDR` = mean(bh_fdr),
    `Bonferroni Power` = mean(bonf_power),
    `BH Power` = mean(bh_power)
  )

summary_long <- summary_stats |>
  pivot_longer(everything(), names_to = "metric", values_to = "value") |>
  separate(metric, into = c("Method", "Measure"), sep = " ", extra = "merge")

ggplot(summary_long, aes(x = Measure, y = value, fill = Method)) +
  geom_col(position = "dodge", width = 0.7) +
  geom_hline(yintercept = 0.05, linetype = "dashed", color = "red") +
  annotate("text", x = 1.5, y = 0.07, label = "Nominal level = 0.05", color = "red", size = 4) +
  scale_fill_manual(values = c("BH" = "steelblue", "Bonferroni" = "coral")) +
  scale_y_continuous(labels = scales::percent, limits = c(0, NA)) +
  labs(x = "", y = "Rate",
       title = "Comparison of Bonferroni (FWER control) vs BH (FDR control)",
       subtitle = paste0("Simulation: m = ", m, " tests, ", (1-pi0)*100, 
                        "% true alternatives, ", n_sims, " replications")) +
  theme(legend.position = "top")
```
...
2. **FWER** = P(at least one false positive); controlled by **Bonferroni**, **Holm**

3. **FDR** = E(proportion of false discoveries); controlled by **BH**, **BY**
...
3. **Size of a test:** P-value tests have exact size $\alpha$ because $P(P \leq \alpha) = \alpha$ when $P \sim \text{Uniform}(0,1)$
